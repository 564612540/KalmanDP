...
if __name__ == '__main__':
    ...
    if args.clipping:
        privacy_engine = PrivacyEngine_Distributed_extending(model, noise_multiplier=noise, grad_accum_steps = acc_step, sample_size= sample_size, batch_size=args.bs, epochs= args.epoch, per_sample_clip=args.clipping, torch_seed_is_fixed=True, num_GPUs=torch.distributed.get_world_size())
    ...
    for E in range(start, args.epoch):
        train(model, train_dl, optimizer, criterion, log_file, epoch = E, log_frequency = args.log_freq, acc_step = acc_step, lr_scheduler=lrscheduler)
        test(model, test_dl, criterion, log_file, epoch = E)
        gc.collect()
